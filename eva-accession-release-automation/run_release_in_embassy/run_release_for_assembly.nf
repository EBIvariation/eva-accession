...
process sort_bgzip_index_release_files {
    ...
    """
    export PYTHONPATH=$params.python_path
    $params.executable.python_interpreter -m run_release_in_embassy.sort_bgzip_index_release_files --bgzip-path $params.executable.bgzip --bcftools-path $params.executable.bcftools --vcf-sort-script-path $params.executable.sort_vcf_sorted_chromosomes --taxonomy-id $params.taxonomy --assembly-accession $params.assembly --assembly-release-folder $params.assembly_folder 1>> $params.log_file 2>&1
    """
}
...
process validate_release_vcf_files {
    ...
    """
    export PYTHONPATH=$params.python_path
    $params.executable.python_interpreter -m run_release_in_embassy.validate_release_vcf_files --private-config-xml-file $params.maven.settings_file --profile $params.maven.environment --taxonomy-id $params.taxonomy --assembly-accession $params.assembly --release-species-inventory-table eva_progress_tracker.clustering_release_tracker --release-version $params.release_version --assembly-release-folder $params.assembly_folder --vcf-validator-path $params.executable.vcf_validator --assembly-checker-path $params.executable.vcf_assembly_checker 1>> $params.log_file 2>&1
    """
}
...
process analyze_vcf_validation_results {
    ...
    """
    export PYTHONPATH=$params.python_path
    $params.executable.python_interpreter -m run_release_in_embassy.analyze_vcf_validation_results --assembly-release-folder $params.assembly_folder --assembly-accession $params.assembly 1>> $params.log_file 2>&1
    """
}
...
process count_rs_ids_in_release_files {
    ...
    """
    export PYTHONPATH=$params.python_path
    $params.executable.python_interpreter -m run_release_in_embassy.count_rs_ids_in_release_files --count-ids-script-path $params.executable.count_ids_in_vcf --taxonomy-id $params.taxonomy --assembly-accession $params.assembly --assembly-release-folder $params.assembly_folder 1>> $params.log_file 2>&1
    """
}
